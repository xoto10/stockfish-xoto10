...
#undef S
  ...
  Score Evaluation<T>::pieces() {
    ...
    constexpr Color     Them = (Us == WHITE ? BLACK : WHITE);
    constexpr Direction Down = (Us == WHITE ? SOUTH : NORTH);
    constexpr Bitboard Centre4 = (Rank4BB | Rank5BB) & (FileDBB | FileEBB);
    ...
    for (Square s = *pl; s != SQ_NONE; s = *++pl)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (bb & s)
                score += Outpost * (Pt == KNIGHT ? 4 : 2) * (bb & s & Centre4 ? 2 : 1);

            else if (Bitboard op = bb & b & ~pos.pieces(Us))
                score += Outpost * (Pt == KNIGHT ? 2 : 1) * (op & Centre4 ? 2 : 1);

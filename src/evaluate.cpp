...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  else
  {
      ...
      nnueComplexity = (  (optimism  > 0 ? 420 * nnueComplexity : 431 * nnueComplexity)    // nnue complexity
                        + (optimism  > 0 ? 407 * abs(psq - nnue) : 325 * abs(psq - nnue))  // pos OR mat adv
                        + (optimism  > 0 ? 42 * int(psq - nnue) : 11 * int(nnue - psq))    // mat if winning
                        ) / 1024;
      ...
      v = (nnue * scale + optimism * (scale - 784)) / 1024;
  }

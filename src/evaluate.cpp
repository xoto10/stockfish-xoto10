...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  else
  {
      ...
      nnueComplexity = (  (optimism  > 0 ? 420 * nnueComplexity : 416 * nnueComplexity)    // nnue complexity
                        + (optimism  > 0 ? 407 * abs(psq - nnue) : 314 * abs(psq - nnue))  // pos OR mat adv
                        + (optimism  > 0 ? 42 * int(psq - nnue) : 10 * int(nnue - psq))    // mat if winning
                        ) / 1024;

...
#define S(mg, eg) make_score(mg, eg)
  ...
  constexpr Score AttackingBishop     = S( 10, 10);
...
#undef S
  ...
  Score Evaluation<T>::pieces() {
    ...
    constexpr Bitboard OurRanks123 = (Us == WHITE ? Rank1BB | Rank2BB | Rank3BB
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (Pt == BISHOP)
            {
                    ...
                    && pos.non_pawn_material() > 13000)
                {
                    ...
                    int n = 0;
                    while (b)
                        if (attacks_bb<BISHOP>(atksq = pop_lsb(&b), pos.pieces(PAWN)) & OurRanks123)
                            score += AttackingBishop * (++n > 1) * (1 + !(pos.pieces(Them, BISHOP) & bb));
                }

...
#undef S
  ...
  Score Evaluation<T>::initiative(Score sc) const {
    int mg = mg_value(sc), eg = eg_value(sc);
    ...
    int v = ((eg > 0 && (mg > -20 || pos.non_pawn_material(WHITE) < 4100))
            - (eg < 0 && (mg < 20 || pos.non_pawn_material(WHITE) < 4100))) * std::max(complexity, -abs(eg));

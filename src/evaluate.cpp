...
#undef S
  ...
  template<Tracing T>
  Score Evaluation<T>::initiative() const {
    ...
    Value rsc = pos.this_thread()->rootMoves[0].score;
    int u = ((rsc > 0) - (rsc < 0)) * std::min(complexity + 50, 0);
    int v = ((rsc > 0) - (rsc < 0)) * complexity;

...
#undef S
  ...
  Score Evaluation<T>::initiative(Value eg) const {
    ...
    int v = 0;
    if (eg > 0)
        v = std::max(complexity + 11 * popcount(pe->real_semiopen() | pe->passed_pawns(WHITE)), int(-eg));
    else if (eg < 0)
        v = -std::max(complexity + 11 * popcount(pe->real_semiopen() | pe->passed_pawns(BLACK)), int(eg));

...
#undef S
  ...
  Score Evaluation<T>::initiative(Value eg) const {
    ...
    int v = 0;
    if (eg > 0)
        v = std::max( complexity + 20 * (kingDangers[WHITE] <= 0 && pos.non_pawn_material(WHITE) > 4000)
                    , int(-eg));
    else if (eg < 0)
        v = -std::max( complexity + 20 * (kingDangers[BLACK] <= 0 && pos.non_pawn_material(BLACK) > 4000)
                     , int(eg));

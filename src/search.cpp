           ...
           && !(limits.depth && mainThread && rootDepth > limits.depth))
    {
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            double fallingEval = (1067 + 223 * (mainThread->bestPreviousAverageScore - bestValue)
                               ...
                               / 10000.0;
            fallingEval = std::clamp(fallingEval, 0.609, 1.632);

            // If the bestMove is stable over several iterations, reduce time accordingly
            timeReduction    = lastBestMoveDepth + 8 < completedDepth ? 1.390 : 0.564;
            double reduction = (1.48 + mainThread->previousTimeReduction) / (2.17 * timeReduction);
            double unstable            = 1 + 2 * std::log(0.1 + completedDepth);
            double bestMoveInstability = 1 + 9.40 * totBestMoveChanges / (threads.size() * unstable);

         ...
         && !(Limits.depth && mainThread && rootDepth / ONE_PLY > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 5 * ONE_PLY)
          {
              ...
              delta = Value(18);
              alpha = std::max(previousScore - (previousScore < -24 ? 28*delta/16 : delta),-VALUE_INFINITE);
              beta  = std::min(previousScore + (previousScore >  66 ? 12*delta/16 : delta), VALUE_INFINITE);
          ...
          while (true)
          {
              ...
              {
                  beta = (alpha + beta) / 2;
                  alpha = std::max(bestValue - (bestValue < -24 ? 28*delta/16 : delta), -VALUE_INFINITE);
                  ...
                  beta = std::min(bestValue + (bestValue > 66 ? 12*delta/16 : delta), VALUE_INFINITE);
              else
                  break;

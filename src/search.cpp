...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // ttHitTotal and ttHitCount are used to calculate the running average of ttHit
    thisThread->ttHitCount += ttHit;
    ++thisThread->ttHitTotal;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (    depth >= 3
          ...
          && (  !captureOrPromotion
              ...
              || cutNode
              || thisThread->ttHitCount * 1024 < 384 * thisThread->ttHitTotal))
      {
          Depth r = reduction(improving, depth, moveCount);

          // Decrease reduction if the ttHit running average is large
          if (thisThread->ttHitCount * 1024 > 544 * thisThread->ttHitTotal)
              r--;

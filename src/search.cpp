...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      {
          double fallingEval = (314 + 7 * (mainThread->bestPreviousScore - bestValue)
                                    + 7 * (mainThread->iterValue[iterIdx] - bestValue)) / 789.7;
          fallingEval = std::clamp(fallingEval, 0.5, 1.5);

          // If the bestMove is stable over several iterations, reduce time accordingly
          timeReduction = lastBestMoveDepth + 9 < completedDepth ? 1.85 : 1.02;
          double reduction = (1.55 + mainThread->previousTimeReduction) / (2.35 * timeReduction);
          ...
          double bestMoveInstability = 1.00 + 2.05 * totBestMoveChanges / Threads.size();

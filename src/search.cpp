  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else
    {
        ...
        if ((ss-1)->currentMove != MOVE_NULL)
            ...
            ss->staticEval = eval = -(ss-1)->staticEval + 2 * (Tempo + int(pos.this_thread()->nodes & 0x2));
    ...
    if (is_ok((ss-1)->currentMove) && !(ss-1)->inCheck && !priorCapture)
    {
        int bonus = std::clamp(-depth * 4 * int((ss-1)->staticEval + ss->staticEval
                                 - 2 * (Tempo + int(pos.this_thread()->nodes & 0x2))), -1000, 1000);
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            (ss-1)->currentMove != MOVE_NULL ? evaluate(pos)
                                             : -(ss-1)->staticEval + 2 * (Tempo + int(pos.this_thread()->nodes & 0x2));

  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else
    {
        ...
        if ((ss-1)->currentMove != MOVE_NULL)
            ...
            ss->staticEval = eval = -(ss-1)->staticEval + 2 * Time.tempo;
    ...
    {
        int bonus = std::clamp(-depth * 4 * int((ss-1)->staticEval + ss->staticEval - 2 * Time.tempo), -1000, 1000);

...
void TimeManagement::init(Search::LimitsType& limits,
    ...
    if (originalTimeAdjust < 0)
        originalTimeAdjust = 0.2128 + 0.1607 * std::log10(timeLeft);
    ...
    if (limits.movestogo == 0)
    {
        ...
        double optConstant  = std::min(0.00318 + 0.000314 * logTimeInSec, 0.00506);
        double maxConstant  = std::max(3.39 + 3.01 * logTimeInSec, 2.93);

        optScale = std::min(0.0126 + std::pow(ply + 3.08, 0.484) * optConstant,

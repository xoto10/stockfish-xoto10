...
namespace {
  ...
  int remaining(int myTime, int myInc, int moveOverhead, int movesToGo,
    ...
    if (movesToGo)
    {
	// allow full overhead for last move of time control, a bit less for previous one,
	// a bit less for previous one, back to 0 for this move; avg = moves*overhead/2
        double mtg = (movesToGo+1.0) / 2.0;

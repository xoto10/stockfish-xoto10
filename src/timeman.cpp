...
void TimeManagement::init(
  Search::LimitsType& limits, Color us, int ply, const OptionsMap& options, int& originalPly) {
    ...
    if (limits.movestogo == 0)
    {
        ...
        double optTime = std::min(0.00381 * logTimeInSec, 0.01187);
        double optPly  = 0.00267 * std::pow(ply + 0.368, 0.496);
        optScale = std::min(0.01151 + optTime + optPly, 0.213 * limits.time[us] / timeLeft)
                 * optExtra;

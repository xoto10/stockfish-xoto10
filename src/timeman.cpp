...
void TimeManagement::init(Search::LimitsType& limits,
    ...
    if (originalTimeAdjust < 0)
        originalTimeAdjust = 0.2150 + 0.1598 * std::log10(timeLeft);
    ...
    if (limits.movestogo == 0)
    {
        ...
        double optConstant  = std::min(0.00323 + 0.000315 * logTimeInSec, 0.00506);
        double maxConstant  = std::max(3.39 + 3.01 * logTimeInSec, 2.93);

        optScale = std::min(0.0124 + std::pow(ply + 3.08, 0.486) * optConstant,

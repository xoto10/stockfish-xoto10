...
public:
  ...
  //Eval:   <0  <6 <12 <18 <24 <30 <36 >=36
          {-22,-10, -2,  1,  1,  1,  0, -2},  //        Diff < -36
          {-15, -6, -1,  1,  1,  2,  0, -1},  // -36 <= Diff < -24
          { -4, -1,  0,  0,  0,  0,  1,  1},  // -24 <= Diff < -12
          {  0,  1,  0,  1, -2, -2, -3,  1},  // -12 <= Diff <   0
          {  0,  2,  0, -2, -3, -3, -1,  2},  //   0 <= Diff <  12
          {  2,  0, -1,  0,  0,  0,  0,  5},  //  12 <= Diff <  24
          {  1,  0, -1,  0,  0,  1,  3,  9},  //  24 <= Diff <  36
          {  1,  0, -1,  0,  1,  2,  6, 12}   //  36 <= Diff
      },
      {            //  Black
  //Eval: <-10 <-4  <2  <8 <14 <20 <26 >=26
          {-25, -8, -4, -6,  0,  1,  0,  1},  //        Diff < -36
          {-18, -5, -2, -4,  1,  1,  0,  2},  // -36 <= Diff < -24
          { -8, -1,  0, -1, -1,  0,  0,  1},  // -24 <= Diff < -12
          {  3, -1, -1, -2,  0, -1,  0,  2},  // -12 <= Diff <   0
          { -1,  1,  1,  1,  0,  0,  1, -2},  //   0 <= Diff <  12
          {  0, -2, -2, -2,  0,  1,  3,  5},  //  12 <= Diff <  24
          { -2,  0,  0,  0,  2,  4,  5,  6},  //  24 <= Diff <  36

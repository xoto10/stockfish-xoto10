...
//constexpr int A[] = {0, 0, 0, 159};
//constexpr int B[] = {154, 154, 154, 0};
...
void TTEntry::save(
  Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t curr_generation) {
    ...
    if (   uint16_t(k) != key16
        || 159 * (b == BOUND_EXACT) + 70 * pv + 32 * (d - DEPTH_NONE)
             + 32 * (v > value16 ? msb(v - value16) + 1 : 0)
           > 32 * depth8 - 128 - 154 * (Bound((genBound8 & BOUND_MASK) >> BOUND_SHIFT) != BOUND_EXACT)
